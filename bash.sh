...
mogrify -format jpg -filter Triangle -define filter:support=2 -thumbnail 650 -quality 95 -unsharp 0.25x0.08+8.3+0.045 -dither None -define jpeg:fancy-upsampling=off -interlace none -colorspace sRGB -path .tmp/ .tmp/*.*
mv .tmp/*.jpg .tmp/../../compressed/; rm -R .tmp/*;

med2x=`ls | grep '_md_2x'` && cp $med2x .tmp/
mogrify -format jpg -filter Triangle -define filter:support=2 -thumbnail 1300 -quality 95 -unsharp 0.25x0.08+8.3+0.045 -dither None -define jpeg:fancy-upsampling=off -interlace none -colorspace sRGB -path .tmp/ .tmp/*.*
mv .tmp/*.jpg .tmp/../../compressed/; rm -R .tmp/*;

#Large
lg=`ls | grep '_lg[^_]'` && cp $lg .tmp/
mogrify -format jpg -filter Triangle -define filter:support=2 -thumbnail 1000 -quality 95 -unsharp 0.25x0.08+8.3+0.045 -dither None -define jpeg:fancy-upsampling=off -interlace none -colorspace sRGB -path .tmp/ .tmp/*.*
mv .tmp/*.jpg .tmp/../../compressed/; rm -R .tmp/*;

lg2x=`ls | grep '_lg_2x'` && cp $lg2x .tmp/
mogrify -format jpg -filter Triangle -define filter:support=2 -thumbnail 2000 -quality 95 -unsharp 0.25x0.08+8.3+0.045 -dither None -define jpeg:fancy-upsampling=off -interlace none -colorspace sRGB -path .tmp/ .tmp/*.*
mv .tmp/*.jpg .tmp/../../compressed/; rm -R .tmp/*;
